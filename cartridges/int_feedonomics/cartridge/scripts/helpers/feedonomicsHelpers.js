...
function generateCSVHeader(exportType) {
    ...
    if (exportType === FConstants.EXPORT_TYPE.CATALOG) {
        ...
        csvHeaderArray.push(FConstants.HEADER_VALUES.ONLINE_FROM);
        csvHeaderArray.push(FConstants.HEADER_VALUES.ONLINE_TO);
        ...
        csvHeaderArray.push(FConstants.HEADER_VALUES.PRODUCT_TYPE);
        csvHeaderArray.push(FConstants.HEADER_VALUES.ONLINE);
        csvHeaderArray.push(FConstants.HEADER_VALUES.CUSTOM_INVENTORTY_FIELDS);
    }
 ...
 * @returns {JSON} JSON of all the custom properties of respective object
 */
function getAllCustomProps(obj) {
    var customJSON = {};
    if (obj) {
        Object.keys(obj.custom).forEach(function (key) {
...
function getDescription(product) {
    var description = {
        short_description: (product.shortDescription ? product.shortDescription.toString() : ''),
        long_description: (product.longDescription ? product.longDescription.toString() : '')
    };
...
/**
 * Return Online From Date
 * @param {dw.catalog.Product} product - Product
 * @returns {string} online from date
 */
function getOnlineFromDate(product) {
    var onlineFrom = product.onlineFrom;
    if (onlineFrom) {
        return onlineFrom.toDateString();
    }
    return '';
}

/**
 * Return Online To Date
 * @param {dw.catalog.Product} product - Product
 * @returns {string} onlne to date
 */
function getOnlineToDate(product) {
    var onlineTo = product.onlineTo;
    if (onlineTo) {
        return onlineTo.toDateString();
    }
    return '';
}
...
/**
 * Return Product Inventory Record
 * @param {dw.catalog.Product} product - Product
 * @returns {dw.catalog.ProductInventoryRecord} inventory record || null
 */
function getProductInventoryRecord(product) {
    var avm = product.availabilityModel;
    if (avm) {
        return avm.inventoryRecord;
    }
    return null;
}
...
function calculatePriceBookPrices(product) {
    ...
    if (priceInfos) {
        ...
        while (priceInfosItr.hasNext()) {
            ...
            priceBookPrice[priceBook.ID] = {
                value: priceInfo.price.value,
            ...
            while (priceBook.parentPriceBook) {
                ...
                var parentPriceBookInfo = priceModel.getPriceBookPriceInfo(priceBook.ID);
                priceBookPrice[priceBook.ID] = {
                    value: priceModel.getPriceBookPrice(priceBook.ID).value,
                    priceInfo: parentPriceBookInfo && parentPriceBookInfo.priceInfo ? parentPriceBookInfo.priceInfo : ''
                };
                priceBookMap.put(priceBook.ID, priceBookPrice);
            }
        }
...
function calculateAllPriceBooksPrices(product) {
    ...
    while (siteAllPriceBooks.hasNext()) {
        ...
        var priceBookPrice = {};
        var priceBookPriceInfo = priceModel.getPriceBookPriceInfo(priceBook.ID);
        priceBookPrice[priceBook.ID] = {
            value: priceModel.getPriceBookPrice(priceBook.ID).value,
            priceInfo: priceBookPriceInfo && priceBookPriceInfo.priceInfo ? priceBookPriceInfo.priceInfo : ''
        };
...
module.exports = {
    ...
    getOnlineFromDate: getOnlineFromDate,
    getOnlineToDate: getOnlineToDate,
    ...
    getProductInventoryRecord: getProductInventoryRecord
};
